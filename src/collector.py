...
def collect_body(
    ...
    try:
        ...
        if err == "login_required":
            update_article_body(article_id, "", "", Status.BODY_BLOCKED)
            return Status.BODY_BLOCKED

        if err == "next_error":
            _save_diagnostic(article_id, session)
            update_article_body(article_id, "", "", Status.BODY_FAILED)
        ...
        if err is not None:
            print(f"[collector] unknown err: {err}")
            update_article_body(article_id, "", "", Status.BODY_FAILED)
            return Status.BODY_FAILED
...
def _save_diagnostic(article_id: str, session: BrowserSession) -> None:
