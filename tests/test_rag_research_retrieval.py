...
def test_build_no_results_records_preserves_questions_and_error():
    runner = load_runner()

    records = runner.build_no_results_records(
        [question_row()],
        top_k=5,
        retrieval_error="collection_missing: goodmorning_chunks",
    )
    ...
    assert records[0]["results"] == []
    assert records[0]["retrieval_status"] == "no_results"
    assert records[0]["retrieval_error"] == "collection_missing: goodmorning_chunks"
...
def test_format_markdown_report_is_human_readable():
    ...
    markdown = runner.format_markdown_report(
        ...
        collection_summary={"collection_exists": True, "points_count": 1},
    )
