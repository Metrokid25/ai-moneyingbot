...
3. `index_tail.py` ↔ `index_tail_realtime.py`는 556줄 포크 — **수정은 반드시 양쪽에**(divergence 사고 2회 전례).
...
2. index_tail 포크 중복(§6-3) — 근본 해법은 공용 모듈 통합.
3. msvcrt Enter-wait 4중 복제 / `daily_archive.fetch_list_rows` 죽은 코드 ~115줄 / `build_daily_archive_command` 미사용.
4. `batch_recollect --estimate 2828`이 15개/페이지 기준(API는 20) — 실시간 수집 무영향.
5. 워치독·CollectLoop 모두 로그온 세션 필요 — **완전 로그오프 상태는 여전히 사각**(재부팅 후 로그인 규칙으로 커버).
