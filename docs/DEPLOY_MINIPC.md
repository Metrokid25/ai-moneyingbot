...
- **SSH에서 등록할 때 계정-SID 매핑 오류 `0x80070534`:** SSH 세션은 로컬 계정이어도
  `USERDOMAIN=WORKGROUP`을 노출할 수 있다. 등록 스크립트는 환경변수를 조합하지 않고 현재 인증된
  Windows identity의 정규 계정명(`컴퓨터명\사용자`)을 사용한다. 임의로 `USERDOMAIN`을 영구 변경하지 말고
  최신 등록 스크립트로 다시 실행한다.
- **색인은 끝났는데 성공 통지가 오래 걸린다:** 마지막 수집글 생존신호는 Archive DB를 변경하거나
  새 인덱스를 만들지 않고 `articles.article_id` 기본키 역순으로 최신 `BODY_COLLECTED` 글을 읽는다.
  구버전의 `ORDER BY saved_at`은 인덱스 없는 16GB 테이블 전체를 읽어 후처리가 수십 분 걸릴 수 있다.
