...
When `--market-schedule --interactive-login` is used, the loop only evaluates
the market schedule. If the current time is `23:00` ~ `06:00`, it waits instead
of running collection. During active windows, it runs the proven
`index_tail.py --collect-after-snapshot --interactive-login` routine followed by
`batch_recollect.py`.
...
## Interactive Login During Collection

The Archive Bot handles login only inside the existing
`index_tail.py --interactive-login` collection flow. Use the mentor teacher
article-list URL with the proven archive routine:

```powershell
.\.venv\Scripts\python.exe scripts\index_tail.py "<멘토선생님 작성글 목록 URL>" --collect-after-snapshot --interactive-login
.\.venv\Scripts\python.exe scripts\batch_recollect.py
```

- Naver home login alone may not be enough to confirm Cafe article-list access.
- If login is required, `index_tail.py --interactive-login` opens the existing manual login flow.
- The user completes Naver login, CAPTCHA, or identity verification directly.
- Continue only after the mentor teacher article list is visible.
...
3. Restart the loop with `.\.venv\Scripts\python.exe scripts\run_daily_archive_loop.py --list-url "<멘토선생님 작성글 목록 URL>" --interactive-login --market-schedule`.
4. If the proven `index_tail.py --interactive-login` flow opens a browser, complete login, CAPTCHA, or identity verification yourself.
5. Confirm the mentor teacher article list is visible, then continue the existing interactive prompt.

Scheduler templates run only `run_daily_archive_loop.py` with `--interactive-login`
and, for the market loop, `--market-schedule`.
...
`daily_archive.py --execute`는 새 목록 수집 실험 경로가 아니라 기존 `index_tail` 방식의 list indexing 경로를 재사용하며, `f-e` 목록 marker readiness 재확인을 포함한다.

## Market Schedule Mode

Use `--market-schedule` when the archive loop should follow the operator's daily market rhythm instead of a fixed interval.
The loop still runs the proven path only:

1. `scripts/index_tail.py "<list-url>" --collect-after-snapshot`
2. `scripts/batch_recollect.py`

Time windows use the local PC time:

- `23:00` ~ `06:00`: stop collection and wait until the next active window.
- `06:00` ~ `07:00`: run every 30 minutes.
- `07:00` ~ `08:00`: run every 10 minutes.
- `08:00` ~ `16:00`: run every 5 minutes.
- `16:00` ~ `18:00`: run every 10 minutes.
...
```powershell
.\.venv\Scripts\python.exe scripts\run_daily_archive_loop.py --market-schedule --list-url "<멘토선생님 작성글 목록 URL>"
```

When `--market-schedule` is set, the time-window interval overrides `--interval-seconds`.
...
Use `--interactive-login` for the proven collection flow: if login is required, the user logs in directly, then PowerShell Enter continues collection.
...
로그인이 필요하면 `index_tail.py --interactive-login`의 기존 흐름에서 브라우저가 열리고 사용자가 직접 네이버 로그인을 완료해야 한다.
아이디/비밀번호 자동 입력, CAPTCHA 우회, 본인인증 우회는 하지 않는다.
CAPTCHA 또는 본인인증이 뜨면 사용자가 직접 처리한다.
로그인이 필요한 경우 콘솔의 기존 interactive prompt를 따른다.

이후 `run_daily_archive_loop.py`는 같은 성공 루틴을 시간표대로 반복한다.
...
2. `.\.venv\Scripts\python.exe scripts\run_daily_archive_loop.py --list-url "<멘토선생님 작성글 목록 URL>" --interactive-login --market-schedule`를 다시 실행한다.
3. `index_tail.py --interactive-login` 흐름에서 열린 브라우저로 네이버 로그인, CAPTCHA, 본인인증을 직접 완료하고 멘토선생님 작성글 목록이 보이는지 확인한다.
4. 콘솔의 기존 interactive prompt를 따른다.
5. `python scripts\run_daily_archive_loop.py --preflight`로 기본 실행 조건을 확인한다.
...
네이버 로그인은 자동으로 만들거나 갱신하지 않으므로 사용자가 브라우저에서 직접 처리해야 한다.
...
로그인이 필요하면 열린 브라우저에서 사용자가 직접 네이버 로그인, CAPTCHA, 본인인증을 완료한다.
로그인 완료 후 콘솔에서 Enter를 눌러 종료한다.
이후 자동 실행 루프도 기존 성공 루틴을 사용한다.
...
루프를 다시 시작하고, `index_tail.py --interactive-login` 흐름이 브라우저를 열면 사용자가 직접 로그인한다. 브라우저에서 글 목록이 보이는지 확인한 뒤 기존 prompt를 따라 진행한다.
...
- 작업 스케줄러는 `run_daily_archive_loop.py`만 실행한다.
...
- `archive.db` 백업 존재 확인
- 네이버 로그인이 필요하면 `index_tail.py --interactive-login` 흐름에서 사용자가 직접 처리
- 멘토선생님 작성글 목록 URL 준비
