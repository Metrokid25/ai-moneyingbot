...
```powershell
python scripts\daily_archive.py --login
```
...
```powershell
python scripts\daily_archive.py --login --browser-profile-dir state\browser_profile
```
...
2. `python scripts\daily_archive.py --login`을 실행한다.
3. 열린 브라우저에서 네이버 로그인, CAPTCHA, 본인인증을 직접 완료한다.
...
작업 스케줄러는 브라우저 수동 로그인 대기 명령인 `daily_archive.py --login`을 실행하지 않는다.
...
사용자가 직접 `python scripts\daily_archive.py --login`을 실행해 로그인 세션을 다시 준비한 뒤, 로그인 완료 후 Enter로 종료하고 루프를 재시작한다.
...
이 래퍼는 무인 자동 실행용이므로 `daily_archive.py --login`을 포함하지 않는다.
...
- `archive.db` 백업 존재 확인
- 네이버 로그인 유지 확인
- `python scripts\daily_archive.py --login`으로 Archive Bot profile 로그인 확인
- 멘토선생님 작성글 목록 URL 준비
